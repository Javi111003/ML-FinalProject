...
A partir de 2014, el \gls{dl} comenzó a aplicarse con redes profundas (\gls{dnn}), mostrando mejoras claras frente a los métodos tradicionales. 
...
En paralelo, se exploraron integraciones más complejas como DMVST-Net, y se mantuvo la relevancia de algoritmos clásicos (\gls{svm}, \gls{knn}, árboles) en tareas de clasificación. 

En el periodo 2019--2021 se diversificaron las técnicas, incorporando clustering y reducción de dimensionalidad (\gls{kmeans}, \gls{dbscan}, \gls{pca}), 
...
Las revisiones recientes concluyen que el aprendizaje profundo moderno (\gls{lstm}, \gls{gnn}, \gls{transformer}) domina en problemas \gls{traficoespaciotemporal} complejos, 
...
Como mostraron las gráficas en cada uno de los servicios estudiados se observan "picos" $\quad$ que pueden describir eventos en los datos (ver Figura \ref{fig:cmp}). 
...
En las horas registradas en el horario entre las 8 y 10 de la mañana (ver Figura \ref{fig:ptr_d}), se refleja un notable consumo, que pudiera estar relacionado con el horario pico del inicio de la jornada laboral por el aumento del trabajo a distancia debido a reuniones matutinas.
...
En las horas registradas se muestra un consumo significativo en el horario entre las 4 y 6 de la madrugada (ver Figura \ref{fig:ptr_v}).
...
utilizando la columna \texttt{ACTUAL\_USAGE} (ver Cuadro \ref{tab:actual_usage_v}) y asumiendo que cada unidad corresponde a un segundo. 
...
En las horas registradas se observa un pequeño aumento del consumo en el horario entre las 10 de la mañana y 12 de la tarde (ver Figura \ref{fig:ptr_s}). No obstante no se distinguen diferencias significativas.
...
\begin{tabular}{|p{3cm}|p{5cm}|}
\hline
\textbf{Received SMS} & \textbf{Valor} \\
\hline
Media Normal & 2.54 \\
\hline
Media Outlier & 25.96 \\
...
En contraste, las variables \texttt{RESERVE[0-10]} no aportan información relevante y no justifican un análisis más profundo. De este modo, se concluye que los aspectos más significativos para el estudio son los relacionados con los usuarios y su nivel de consumo. 
...
\subsection{Contexto Tradicional y Motivación}

Tradicionalmente, el ciclo de vida de un proyecto de Machine Learning (ML) involucra fases iterativas y secuenciales como:
\begin{itemize}
...
La falta de estandarización, derivada de la naturaleza diversa de los problemas abordados por el ML, junto con el alto costo en \textit{horas de trabajo especializado} y recursos de cómputo, ha generado históricamente ineficiencias significativas. Entre las consecuencias más notables se encuentran:
\begin{itemize}
    \item Procesos mayormente manuales y propensos a errores humanos.
    \item Experimentación limitada y poco exhaustiva por restricciones de tiempo.
    \item Soluciones con baja reproducibilidad y dificultad para auditar.
\end{itemize}
...
En aras de estos objetivos y para abordar las necesidades específicas de este proyecto ---en particular, el trabajo con \textbf{series de tiempo}---, se optó por diseñar e implementar un \textit{mini-framework} de AutoML especializado.
...
\subsection{Conclusión y Lecciones Aprendidas}

La implementación de un mini-framework de AutoML especializado demostró ser una herramienta valiosa dentro del proyecto. No solo \textbf{agilizó el ciclo de experimentación}, sino que también \textbf{proporcionó una base objetiva y reproducible} para la selección del modelo final. La experiencia reafirma que, si bien el AutoML no reemplaza al experto, lo potencia al permitirle centrarse en la formulación del problema, el diseño de características de alto nivel y la interpretación de resultados, mientras automatiza la búsqueda extensiva y sistemática en el espacio de soluciones técnicas.
